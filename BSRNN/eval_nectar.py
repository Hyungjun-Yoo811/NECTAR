"""
Evaluates the NECTAR backbone (configs/nectar.yml, clue_mode "v2") against
several negative-clue constructions it was NOT trained with (Table 1's
Oracle Utterance / Oracle Centroid / Centroid Approx. / NECTAR / Embedding
Approx. rows -- see CASE_GROUPS and each case's block below). Positive half
is always norm(raw(target_enroll)). Usage: `cd BSRNN && python eval_nectar.py
[--mix_window 3.0] [--enroll_window 3.0] [--cases ...] [--estimator_run_dirs ...]`.
"""
...
from train import (
    ...
    lookup_centroid_embs,
    lookup_pair_centroid_embs,
    run_csm,
)
...
DEFAULT_CFG = "configs/nectar_noisy.yml"
TEST_SPEAKER_CENTROID_CACHE = os.path.join(
    _REPO_ROOT, "dataset/data/centroid_cache/without_norm/test_speaker_centroids.pt")
TEST_PAIR_CENTROID_CACHE = os.path.join(
    _REPO_ROOT, "dataset/data/centroid_cache/without_norm/mixture_pair_centroids_test.pt")
DEFAULT_ESTIMATOR_RUN_DIRS = [os.path.join(_REPO_ROOT, "csm/exp/csm/csm")]
DEFAULT_CKPT_NAME = "CKPT+best"
CASE_GROUPS = [
    "oracle_utterance", "oracle_centroid", "oracle_superposition",
    "estimator", "raw_sup_2x",
]
...
def main():
    ...
    p.add_argument("--comparison_output", default=None,
                   help="Default: <exp_dir>/eval_nectar/comparison_3.0.csv")
    ...
    # ── Cases 1-3: oracle recipes, computed directly below (no training-style
    # config to load) -- only the SELECTED ones' resources are loaded (skips
    # reading a centroid cache entirely for a case that wasn't asked for).
    run_oracle_utterance = "oracle_utterance" in args.cases
    run_oracle_centroid = "oracle_centroid" in args.cases
    run_oracle_superposition = "oracle_superposition" in args.cases
    if run_oracle_utterance:
        print("Case       : oracle_utterance (norm(raw(interference_target)))")
    test_centroids = (
        load_speaker_centroids(TEST_SPEAKER_CENTROID_CACHE)
        if run_oracle_centroid or run_oracle_superposition else None
    )
    if run_oracle_centroid:
        print("Case       : oracle_centroid (interference speaker's oracle centroid)")
    test_pair_centroids = load_pair_centroids(TEST_PAIR_CENTROID_CACHE) if run_oracle_superposition else None
    if run_oracle_superposition:
        print("Case       : oracle_superposition (2*mixture_pair_centroid - target_centroid)")

    # ── Case 4: one per --estimator_run_dirs, formula fixed at
    ...
    run_raw_sup_2x = "raw_sup_2x" in args.cases
    if run_raw_sup_2x:
        print(f"Case       : raw_sup_2x (2*norm(raw(mixture)) - norm(raw(target_enroll)))")
    ...
    case_names = []
    if run_oracle_utterance:
        case_names.append("oracle_utterance")
    if run_oracle_centroid:
        case_names.append("oracle_centroid")
    if run_oracle_superposition:
        case_names.append("oracle_superposition")
    case_names += [c["name"] for c in estimator_cases]
    if run_raw_sup_2x:
        case_names.append("raw_sup_2x")
    if not case_names:
        raise ValueError("--cases selected nothing to evaluate.")
    audio_dir = os.path.join(exp_dir, "eval_nectar", "audio_samples")
    audio_saved = 0
    ...
    with torch.no_grad():
        for (mix_wavs, enroll_wavs, interference_enroll_wavs, interference_target_wavs,
             target_wavs, keys, interference_spk_ids, target_spk_ids) in tqdm(test_loader, desc="eval_nectar"):
            ...
            if run_oracle_centroid:
                # Interference speaker's oracle centroid, magnitude-carrying
                # (never normalized) -- the oracle-identity ceiling.
                neg = lookup_centroid_embs(interference_spk_ids, test_centroids, device)
                spk_embs["oracle_centroid"] = build_spk_embs(pos, neg)
            if run_oracle_superposition:
                # 2*mixture_pair_centroid - target_centroid, the correct
                # algebraic inversion of mixture ~= 0.5*target + 0.5*interference.
                # Magnitude-carrying (never normalized).
                target_centroid = lookup_centroid_embs(target_spk_ids, test_centroids, device)
                mix_pair_centroid = lookup_pair_centroid_embs(
                    target_spk_ids, interference_spk_ids, test_pair_centroids, device)
                neg = 2 * mix_pair_centroid - target_centroid
                spk_embs["oracle_superposition"] = build_spk_embs(pos, neg)
            ...
            if run_raw_sup_2x:
                raw_mix = extract_embeddings(mix_wavs, campplus, fbank, device)
                mix_n = F.normalize(raw_mix, dim=-1)
                spk_embs["raw_sup_2x"] = build_spk_embs(pos, 2 * mix_n - pos)
    ...
    for name in case_names:
        agg = save_case(
            all_rows[name], os.path.join(exp_dir, "eval_nectar", name), checkpoint,
            extra={"backbone_cfg": args.cfg, "backbone_clue_mode": backbone_clue_mode, "case": name},
        )
        agg["case"] = name
        results.append(agg)

    comparison_output = args.comparison_output or os.path.join(exp_dir, "eval_nectar", "comparison_3.0.csv")
